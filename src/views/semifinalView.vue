...
function confirmAssignFinalist() {
  showAssignDialog.value = true
}

function assignFinalist() {
  if (leader.value) store.assignFinalist(leader.value.id)
  showAssignDialog.value = false
}

function confirmClearFinalist() {
  showClearDialog.value = true
}

function clearFinalist() {
  ...
  <div class="semifinal-container">
    <div class="header">
      ...
      <div class="header-right">
        <div v-if="assignedFinalist" class="assigned-badge">
          ...
          <q-btn flat dense round icon="mdi-close" color="white" size="sm" @click="confirmClearFinalist">
        ...
        <template v-else>
          ...
          <q-btn v-if="leader" unelevated no-caps color="secondary" text-color="dark" icon="mdi-trophy-award"
            label="Asignar Finalista" class="assign-btn" @click="confirmAssignFinalist" />
        </template>
      </div>
    </div>
    ...
    <div class="participants-grid">
      <ParticipantCard v-for="participant in participants" :key="participant.id" :participant-id="participant.id"
        ...
        :is-leader="!assignedFinalist && leader?.id === participant.id" class="participant-card-item" />
    </div>
      ...
      <q-card class="confirm-card" style="min-width: 400px">
        ...
        <q-card-section>
          ...
          <div class="text-caption text-grey-7 q-mt-sm">
            ...
            El finalista inicia la final con puntaje en cero. Esta decisión es reversible.
          </div>
        </q-card-section>
        <q-card-actions align="right">
          <q-btn flat label="Cancelar" color="primary" v-close-popup />
          <q-btn unelevated label="Confirmar" color="secondary" text-color="dark" @click="assignFinalist" />
        </q-card-actions>
      </q-card>
    </q-dialog>
    ...
    <q-dialog v-model="showClearDialog">
      <q-card class="confirm-card" style="min-width: 380px">
        ...
        <q-card-actions align="right">
          <q-btn flat label="Cancelar" color="primary" v-close-popup />
          <q-btn unelevated label="Quitar" color="negative" @click="clearFinalist" />
        </q-card-actions>
      </q-card>
    </q-dialog>
  </div>
