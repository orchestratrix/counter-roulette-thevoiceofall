  ...
  <div class="final-container">
    ...
    <div v-if="!canShowFinal" class="warning-panel">
      ...
      <div class="warning-actions">
        <q-btn
          ...
          label="Ir a Semifinal Caribe"
          to="/semifinal-caribe"
        />
        <q-btn
          v-if="!finalists[1]"
          unelevated color="secondary" text-color="dark" no-caps
          label="Ir a Semifinal Andina"
          to="/semifinal-andina"
        />
      </div>
    </div>
    ...
    <template v-else>
      ...
      <div class="finalists-grid">
        <ParticipantCard
          :participant-id="finalists[0].id"
          :show-controls="!store.championId"
          :is-champion="store.championId === finalists[0].id"
          :is-finalist="!store.championId"
          :is-eliminated="store.championId && store.championId !== finalists[0].id"
          class="final-card"
        />
        <ParticipantCard
          :participant-id="finalists[1].id"
          :show-controls="!store.championId"
          :is-champion="store.championId === finalists[1].id"
          :is-finalist="!store.championId"
          :is-eliminated="store.championId && store.championId !== finalists[1].id"
          class="final-card"
        />
      </div>
...
const finalLeader = computed(() => {
  if (!canShowFinal.value) return null
...
const championName = computed(
  () => store.byId(store.championId)?.name || ''
)

const capitalize = (s) => s.charAt(0).toUpperCase() + s.slice(1)
const goHome = () => router.push('/')
const confirmCrown = () => { showCrownDialog.value = true }
...
.versus-scoreboard
  ...
  .versus-divider
    .vs-label
      ...
      letter-spacing: 2px
      text-shadow: 0 0 16px rgba(54, 192, 232, 0.4)
...
.action-bar
  ...
  .crown-btn
    ...
    box-shadow: 0 8px 28px rgba(255, 204, 41, 0.5), 0 0 0 0 rgba(255, 204, 41, 0.4) !important
...
.coronation-overlay
  ...
  .spotlight
    ...
    background: radial-gradient(circle, rgba(255, 204, 41, 0.35) 0%, rgba(255, 204, 41, 0.15) 20%, transparent 50%)
  ...
  .rays
    ...
    height: 200vmax
    transform: translate(-50%, -50%)
    background: conic-gradient(from 0deg, transparent 0deg, rgba(255, 204, 41, 0.1) 10deg, transparent 20deg, transparent 40deg, rgba(255, 204, 41, 0.08) 50deg, transparent 60deg, transparent 90deg, rgba(255, 204, 41, 0.1) 100deg, transparent 110deg, transparent 140deg, rgba(255, 204, 41, 0.08) 150deg, transparent 160deg, transparent 180deg, rgba(255, 204, 41, 0.1) 190deg, transparent 200deg, transparent 220deg, rgba(255, 204, 41, 0.08) 230deg, transparent 240deg, transparent 270deg, rgba(255, 204, 41, 0.1) 280deg, transparent 290deg, transparent 320deg, rgba(255, 204, 41, 0.08) 330deg, transparent 340deg, transparent 360deg)
